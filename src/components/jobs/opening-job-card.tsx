    ...
    <div className='relative flex min-w-[470px] max-w-[800px] flex-1 gap-x-4 overflow-hidden rounded-3xl bg-card p-5 shadow md:gap-x-8'>
      ...
      <div className='flex flex-1 flex-col'>
        <div className='flex w-full items-center justify-between'>
          ...
          <ApplyJobButton jobCode={openingJob.jobCode} recruitmentDriveCode={openingJob.recruitmentDriveCode} />
        </div>
...
const ApplyJobButton = ({ jobCode, recruitmentDriveCode }: ApplyJobButtonProps) => {
  const { role } = useAuthContext()

  if (role === 'Guest') {
    return (
      <Button variant='ghost' className='text-gradient text-base' asChild>
        <Link to='/sign-in'>Apply Job</Link>
      </Button>
    )
  }
  if (role === 'Candidate') {
    return <DialogApplyJob jobCode={jobCode} recruitmentDriveCode={recruitmentDriveCode} />
  }
